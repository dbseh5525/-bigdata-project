...
if st.session_state.step == 1:
    ...
    name = st.text_input('이름', value=st.session_state.get('saved_name', ''), key='name')
    age = st.number_input('나이', min_value=1, max_value=100, value=st.session_state.get('saved_age', 20), key='age')
...
# ---- Step 2: 관심 분야 ----
    ...
    with col2:
        if st.button('다음 →'):
            st.session_state.saved_interests = st.session_state.interests
...
# ---- Step 3: 확인 ----
    ...
    with col2:
        if st.button('✅ 제출'):
            ...
            # 초기화
            for key in ['step', 'name', 'age', 'interests', 'saved_name', 'saved_age', 'saved_interests']:
